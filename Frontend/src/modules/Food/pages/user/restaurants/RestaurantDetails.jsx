...
function RestaurantDetailsContent() {
  ...
  const [restaurantOffersList, setRestaurantOffersList] = useState([])
  ...
  const getDishTotalQuantity = (item) => {
    const itemId = item?.id || item?._id
    if (!itemId) return 0
    let total = 0
    cart.forEach((cartItem) => {
      if (String(cartItem.itemId || "") === String(itemId) && cartItem.restaurant === restaurant?.name) {
  ...
  // Lock body scroll when any overlay/sheet/modal is open
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    const fetchOffers = async () => {
      try {
        const response = await restaurantAPI.getPublicOffers({ restaurantId })
        const offers = response?.data?.data?.allOffers || response?.data?.allOffers || []
        setRestaurantOffersList(offers)
        
        setRestaurant(prev => {
          if (!prev) return prev
          const existingCoupons = prev.restaurantOffers?.coupons || []
          if (existingCoupons.length === offers.length) {
            return prev
          }
          return {
            ...prev,
        ...
        debugWarn("Failed to fetch public offers for restaurant:", err)
      }
    }
    fetchOffers()
  }, [restaurant?.id])
  ...
  const getActiveFilterCount = () => {
    ...
    // When vegMode is ON, vegNonVeg is redundant — don't count it
    if (!vegMode && filters.vegNonVeg) count++
  ...
  const handleItemClick = (item) => {
    ...
    setModalQuantity(cartQty > 0 ? cartQty : 1)
  ...
  const flatFilteredItems = useMemo(() => {
    ...
    let categoryFiltered = uniqueItems
    if (selectedMenuCategory !== "all") {
      categoryFiltered = uniqueItems.filter(item => {
        const parentSection = restaurant.menuSections.find(section => {
          if (isRecommendedSection(section)) return false
          const hasDirect = section.items?.some(it => String(it.id) === String(item.id))
          const hasSub = section.subsections?.some(sub => sub.items?.some(it => String(it.id) === String(item.id)))
          return hasDirect || hasSub
        })
        if (!parentSection) return false
        const sectionCategoryId = normalizeMenuCategoryId(parentSection.categoryId || getSectionDisplayName(parentSection))
  ...
  const highlightOffers = useMemo(() => {
    const list = []
    
    // Add real offers from the fetched offers list
    if (Array.isArray(restaurantOffersList) && restaurantOffersList.length > 0) {
      restaurantOffersList.forEach((offer) => {
        if (offer.title) {
          list.push(offer.title)
        } else if (offer.couponCode) {
          list.push(`Use code ${offer.couponCode}`)
        }
      })
    }
    
    // Fallback only if there are actual offers/coupons on the restaurant object
    if (list.length === 0 && Array.isArray(restaurant?.offers) && restaurant.offers.length > 0) {
    ...
    // Also include the offerText if it's explicitly set and not the default fallback "FLAT 50% OFF"
    if (list.length === 0 && restaurant?.offerText && restaurant.offerText !== "FLAT 50% OFF") {
      list.push(restaurant.offerText)
    }
    
    return list
  }, [restaurantOffersList, restaurant?.offers, restaurant?.offerText])
  ...
  const renderItemCard = (item, prefix = "direct") => {
    const quantity = getDishTotalQuantity(item)
    ...
    return (
      <div
        ...
        className={`flex gap-4 p-4 border-b border-gray-100 last:border-none relative cursor-pointer transition-all duration-500 ${
          ...
          <div className="flex gap-4 mt-3 md:hidden">
            <button
              ...
              className={`p-1.5 border rounded-lg hover:bg-gray-50 dark:hover:bg-gray-800 transition-colors ${
                isDishFavorite(item.id, restaurant?.restaurantId || restaurant?._id || restaurant?.id)
            ...
            {hasActiveMenuFilters ? (
              ...
              flatFilteredItems.length === 0 ? (
                ...
                <div className="space-y-0">
                  {flatFilteredItems.map((item) => renderItemCard(item, "flat"))}
                </div>
              )
            ) : (
              // Original grouped categories view
              <>
                ...
                {filteredSections.map(({ section, originalIndex }, sectionIndex) => {
                  ...
                  return (
                    <div key={sectionIndex} id={sectionId} className="space-y-1 scroll-mt-20">
                      ...
                      {isRecommended && (
                        <div
                          className="flex items-center justify-between cursor-pointer py-2 hover:bg-gray-50 dark:hover:bg-gray-800/30 px-2 rounded-xl transition-all"
                          onClick={() => {
                            setExpandedSections((prev) => {
                      ...
                      {!isRecommended && (
                        ...
                        >
                          <div className="space-y-1">
                            ...
                            {section.subtitle && (
                              <span className="text-sm text-blue-600 dark:text-blue-400 underline block">
                                {section.subtitle}
                              </span>
                            )}
                          </div>
                          ...
                          {sectionSubsections.map((subsection, subIndex) => {
                            ...
                            return (
                              <div key={subIndex} className="space-y-4">
                                {/* Subsection Header */}
                                <div
                                  className="flex items-center justify-between cursor-pointer py-1.5 hover:bg-gray-50 dark:hover:bg-gray-800/30 px-2 rounded-lg transition-all"
                                  onClick={() => {
                                    setExpandedSections((prev) => {
                                      const newSet = new Set(prev)
              ...
              <>
                ...
                >
                  ...
                  <div className="flex-1 overflow-y-auto px-4 py-3 space-y-4">
                    ...
                    {/* Veg/Non-veg preference — hidden when vegMode is ON (it already filters globally) */}
                    {!vegMode && (
                      <div className="space-y-2">
                      ...
                      >
                        ...
                        <div className="flex items-center gap-1">
                          ...
                          <span className="text-base font-bold">
                            {hasFoodVariants(selectedItem)
                              ? `${getVariantForDish(selectedItem, selectedVariantId)?.name || "Default"} · ${RUPEE_SYMBOL}${Math.round((getVariantForDish(selectedItem, selectedVariantId)?.price || selectedItem.price) * modalQuantity)}`
                              : `${RUPEE_SYMBOL}${Math.round(selectedItem.price * modalQuantity)}`}
                          </span>
                        </div>
                      </Button>
                    </div>
