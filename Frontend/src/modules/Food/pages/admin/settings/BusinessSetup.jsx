...
export default function BusinessSetup() {
  ...
  const handleSave = async () => {
    try {
      ...
      if (formData.state.trim() && /\d/.test(formData.state.trim())) {
        toast.error("State name cannot contain numbers");
        return;
      }

      if (formData.pincode.trim() && !/^\d{6}$/.test(formData.pincode.trim())) {
        toast.error("Please enter a valid pincode (exactly 6 digits)");
        return;
      }
        ...
        <div className="bg-white rounded-lg shadow-sm border border-slate-200">
          ...
          <div className="px-4 py-4 border-b border-slate-100">
            ...
            <div className="border-t border-slate-100 my-4 pt-4">
              <h4 className="text-xs font-bold text-slate-900 mb-3 uppercase tracking-wider">TDS settings</h4>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  <label className="block text-xs font-semibold text-slate-700 mb-1.5">
                    Restaurant TDS Percentage (%)
                  </label>
                  <input
                    type="number"
                    min="0"
                    ...
                    step="0.01"
                    placeholder="e.g. 1.0"
                    value={formData.restaurantTdsPercentage}
                    onChange={(e) => {
                      const val = e.target.value === "" ? "" : Math.max(0, Math.min(100, parseFloat(e.target.value) || 0));
                      handleInputChange("restaurantTdsPercentage", val);
                    }}
                    className="w-full px-3 py-2 text-xs border border-slate-300 rounded-lg bg-white focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
                  />
                </div>
                ...
                <div>
                  <label className="block text-xs font-semibold text-slate-700 mb-1.5">
                    Delivery Partner TDS Percentage (%)
                  </label>
                  <input
                    type="number"
                    min="0"
                    ...
                    step="0.01"
                    placeholder="e.g. 2.0"
                    value={formData.deliveryBoyTdsPercentage}
                    onChange={(e) => {
                      const val = e.target.value === "" ? "" : Math.max(0, Math.min(100, parseFloat(e.target.value) || 0));
