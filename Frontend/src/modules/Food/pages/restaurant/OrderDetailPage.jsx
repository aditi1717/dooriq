      ...
      <div className="p-4 space-y-4 max-w-2xl mx-auto">
        ...
        <div className="bg-white rounded-3xl p-6 border border-gray-100 shadow-sm shadow-gray-100/50 space-y-4">
          <div className="flex items-center gap-3 mb-2">
            ...
            <h3 className="text-sm font-black text-gray-900">Payment Summary</h3>
          </div>
          ...
          <div className="space-y-3">
            ...
            <div className="pt-2">
              {(() => {
                ...
                const isPaid = ["captured", "paid", "authorized", "settled"].includes(pStatus);
                const isCod = pMethod === "cash" || pMethod === "cod";
                
                return (
                  <div className={`flex items-center gap-2 px-3 py-2 rounded-xl border ${isPaid ? 'bg-emerald-50 border-emerald-100 text-emerald-700' : isCod ? 'bg-amber-50 border-amber-100 text-amber-700' : 'bg-rose-50 border-rose-100 text-rose-700'}`}>
                    <div className={`w-2 h-2 rounded-full animate-pulse ${isPaid ? 'bg-emerald-500' : isCod ? 'bg-amber-500' : 'bg-rose-500'}`} />
                    <span className="text-[10px] font-black uppercase tracking-[0.05em]">
                      {isPaid ? 'Payment Successful' : isCod ? 'Cash on Delivery' : 'Payment ' + (pStatus || 'Pending')}
                    </span>
                  </div>
                );
              })()}
